...
def test_mesh_contract_default_off_qnm_law() -> None:
    ...
    assert 'MESH_PRODUCT = "shadowlock"' in MESH
...
def test_mesh_pointer_and_openapi_helpers() -> None:
    ...
    assert "shadowlock_mesh_" in MESH
...
def test_mesh_proxies_via_aziel_runtime() -> None:
    ...
    assert 'id = "4f4dcb1c12ff46cebec25a3cdd191ec4"' in WRANGLER
...
def test_home_live_nodes_strip_no_node_gate() -> None:
    assert 'id="meshStrip"' in PAGE
    assert 'id="meshLiveCount"' in PAGE
    assert 'id="meshLine"' in PAGE
    assert "Live Nodes" in PAGE
    assert "QNM-BUILD-1.0" in PAGE
    assert "No Node Gate" in PAGE
    assert "No auto-heal" in PAGE
    assert "Not an anonymity network" in PAGE
    assert "/v1/mesh" in PAGE
    assert 'product: "shadowlock"' in PAGE
    assert 'id="node-gate"' not in PAGE
    assert 'href="/node-gate"' not in PAGE
    assert "auto-heal this node" not in PAGE
